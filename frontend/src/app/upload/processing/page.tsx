  ...
  useEffect(() => {
    ...
    const pollStatus = async () => {
      try {
        ...
        if (!response.ok) {
          console.warn(`[MigrantShield] Status poll returned ${response.status}`);
          return;
        }
        ...
        if (data.status === 'failed') {
          clearAllTimers();
          setErrorMessage(data.error_reason ?? 'Analysis pipeline encountered an error.');
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1 leading-relaxed">
                  Your contract was uploaded successfully. The AI analysis engine
                  is taking longer than expected. This is usually caused by high
                  server load or a large document.
                </p>
              </div>
            </div>
            <div className="mt-5 pt-4 border-t border-slate-100">
              <p className="text-xs text-slate-400">Reference ID</p>
              <p className="text-xs font-mono text-slate-600 mt-1 break-all">{contractId}</p>
            </div>
          </div>

          <div className="mt-4 p-4 rounded-lg bg-amber-50 border border-amber-200">
            <p className="text-xs font-medium text-amber-800">
              Your analysis may still complete in the background.
              Check your dashboard in a few minutes, or retry the analysis below.
            </p>
          </div>
    ...
    <div className="min-h-screen bg-slate-50">
      <main className="max-w-lg mx-auto px-4 py-12">
        <div className="mb-10">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Please keep this screen open. Our AI engine is scanning your contract
            for labour rights violations.
          </p>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6">
          ...
          <div className="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden">
            <div
              ...
              style={{
                width: pollingStatus === 'completed' ? '100%' : `${progressPct}%`,
              }}
            />
          </div>
        ...
        <div className="mt-6 p-4 rounded-lg bg-slate-100 border border-slate-200">
          <p className="text-xs text-slate-500 leading-relaxed">
            Our AI engine is evaluating your contract against international labour
            law standards — checking for illegal recruitment fees, passport
            confiscation clauses, forced wage deductions, and termination threats.
