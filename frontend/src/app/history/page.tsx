...
import { CheckCircle, Loader2, XCircle, Clock, Search, X, RefreshCw } from 'lucide-react';
...
function getStatusMeta(status: Status) {
  switch (status) {
    case 'completed': return { icon: <CheckCircle size={14} className="text-emerald-500" />, color: 'text-emerald-600', bg: 'bg-emerald-50 border-emerald-200' };
    case 'processing': return { icon: <Loader2 size={14} className="text-amber-500 animate-spin" />, color: 'text-amber-600', bg: 'bg-amber-50 border-amber-200' };
    case 'queued': return { icon: <Clock size={14} className="text-slate-400" />, color: 'text-slate-500', bg: 'bg-slate-50 border-slate-200' };
    case 'failed': return { icon: <XCircle size={14} className="text-red-500" />, color: 'text-red-600', bg: 'bg-red-50 border-red-200' };
    default: return { icon: null, color: 'text-slate-400', bg: 'bg-slate-50 border-slate-200' };
  }
}
...
export default function HistoryPage() {
  ...
  useEffect(() => {
    fetchContracts();
    const channel = supabase.channel('history')
      ...
      <div className="px-4 py-4">
        ...
        {!loading && filtered.length === 0 && (
          <div className="text-center py-20">
            <p className="text-slate-900 font-semibold text-base">{t.emptyHeading}</p>
          ...
          <div className="flex flex-col gap-3">
            {paginated.map(contract => {
              ...
              return (
                ...
                >
                  <button
                    onClick={() => clickable && router.push(`/report/${contract.id}`)}
                    disabled={!clickable}
                    ...
                    <div className="w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center shrink-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">
                          {contract.country ?? t.notSpecified} · {contract.upload_date?.slice(0, 10)}
                        </p>
                      </div>
                      <div className="flex flex-col items-end gap-1.5 shrink-0">
                        <span className={`flex items-center gap-1 text-xs font-medium border rounded-full px-2 py-0.5 ${meta.bg} ${meta.color}`}>
                          {meta.icon}
                          {tDash[`status${contract.status.charAt(0).toUpperCase() + contract.status.slice(1)}` as keyof typeof tDash]}
                        </span>
                        {contract.risk_score != null && (
                          <span className={`text-xs font-semibold ${getRiskColor(contract.risk_score)}`}>
                  ...
                  {/* Retry button for failed contracts */}
                  {isFailed && (
