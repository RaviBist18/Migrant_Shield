    ...
    id_line = Table(
        [
            [
                ...
                Paragraph(
                    f"Analysed: {analysed_str}",
                    _s("adt", FR, 8, 11, SLATE_400, align=TA_RIGHT),
                ),
            ]
        ],
...
def _build_meta_card(contract, FR, FBold, is_rtl):
    rows = [
        [
            ...
            Paragraph(
                _t(contract.get("worker_name") or "—", is_rtl),
                _s("mv", FBold, 9, 12, SLATE_900, TA_RIGHT),
            ),
        ],
        [
            Paragraph(_t("Employer", is_rtl), _s("ml", FR, 9, 12, SLATE_500)),
            Paragraph(
                _t(contract.get("employer_name") or "—", is_rtl),
                _s("mv", FBold, 9, 12, SLATE_900, TA_RIGHT),
            ),
        ],
        [
            ...
            Paragraph(
                _t(contract.get("country") or "—", is_rtl),
                _s("mv", FBold, 9, 12, SLATE_900, TA_RIGHT),
            ),
        ],
        [
            ...
            Paragraph(
                contract.get("original_filename") or "—",
                _s("mv", FR, 9, 12, SLATE_700, TA_RIGHT),
            ),
        ],
    ]
...
def _build_score_card(risk_score, critical_count, warning_count, info_count, FR, FBold):
    ...
    score_para = Paragraph(
        str(score_val), _s("sn", FBold, 32, 36, vcfg["text"], TA_CENTER)
    )
    denom_para = Paragraph("/ 100", _s("sd", FR, 10, 13, SLATE_400, TA_CENTER))
    verdict_para = Paragraph(
        vcfg["label"], _s("vl", FBold, 11, 14, vcfg["text"], TA_CENTER)
    )

    score_col = Table(
        [[score_para], [denom_para], [verdict_para]], colWidths=[CONTENT_W * 0.3]
    )
    score_col.setStyle(
        TableStyle(
            [
    ...
    outer_row = Table(
        [[score_col, badges]], colWidths=[CONTENT_W * 0.3, CONTENT_W * 0.7]
    )
    outer_row.setStyle(
        TableStyle(
            [
...
def _build_flag_card(flag, idx, FR, FBold, is_rtl):
    ...
    title_p = Paragraph(
        _t(f"{idx}. {flag_title}", is_rtl), _s("ft", FBold, 10, 14, cfg["text"])
    )
    header_row = Table(
        [[badge_t, title_p]], colWidths=[18 * mm, CONTENT_W - 18 * mm - 24]
    )
    header_row.setStyle(
        TableStyle(
            [
    ...
    clause_row = Table([[accent_bar, clause_inner]], colWidths=[3, CONTENT_W - 24 - 3])
    ...
    if legal_refs:
        ...
        legal_block = Paragraph(
            _t(f"📋 {refs_str}", is_rtl), _s("lr", FR, 8, 11, SLATE_400)
        )
    ...
    inner_rows = [
        ...
        [
            Paragraph(
                "📄 Extracted Contract Clause", _s("sec_l", FBold, 8, 11, SLATE_400)
            )
        ],
        [Spacer(1, 3)],
        [clause_row],
        ...
        [
            Paragraph(
                "📖 Plain Language Explanation", _s("sec_l2", FBold, 8, 11, SLATE_400)
            )
        ],
        [Spacer(1, 3)],
    ...
    if legal_block:
        inner_rows += [
            ...
            [
                Paragraph(
                    "📋 Regulatory Reference", _s("sec_l4", FBold, 8, 11, SLATE_400)
                )
            ],
            [Spacer(1, 2)],
            [legal_block],
...
def _build_contacts(FR, FBold):
    header_p = Paragraph(
        "🆘 Emergency Resource & Legal Advocacy Hub", _s("ch", FBold, 10, 14, WHITE)
    )
    rows = [
        [header_p, ""],
        [
            Paragraph(
                ...
                _s("cn", FBold, 9, 12, SLATE_900),
            ),
            Paragraph(
                "www.ilo.org  |  +41 22 799 6111", _s("cv", FR, 9, 12, SLATE_500)
            ),
        ],
        [
            Paragraph(
                "Pravasi Nepali Coordination Committee (PNCC)",
                _s("cn2", FBold, 9, 12, SLATE_900),
            ),
            Paragraph(
                "www.pncc.org.np  |  +977-1-4004747", _s("cv2", FR, 9, 12, SLATE_500)
            ),
        ],
        [
            Paragraph(
                "Nepal Dept. of Foreign Employment (DOFE)",
                _s("cn3", FBold, 9, 12, SLATE_900),
            ),
            Paragraph(
                "www.dofe.gov.np  |  +977-1-4102390", _s("cv3", FR, 9, 12, SLATE_500)
            ),
        ],
        [
            Paragraph(
                "Migrant Forum in Asia (MFA)", _s("cn4", FBold, 9, 12, SLATE_900)
            ),
            Paragraph(
                "www.mfasia.org  |  info@mfasia.org", _s("cv4", FR, 9, 12, SLATE_500)
            ),
        ],
    ]
...
def generate_pdf(contract_data: dict, flags: list, language: str = "en") -> bytes:
    ...
    doc.contract_id_str = str(
        contract_data.get("contract_id") or contract_data.get("id") or ""
    )
    ...
    critical_count = sum(
        1 for f in flags if (f.get("severity") or "").lower() == "critical"
    )
    warning_count = sum(
        1 for f in flags if (f.get("severity") or "").lower() == "warning"
    )
    info_count = sum(1 for f in flags if (f.get("severity") or "").lower() == "info")

    story.append(
        _build_score_card(
            risk_score, critical_count, warning_count, info_count, FR, FBold
        )
    )
    ...
    if flags:
        ...
        sorted_flags = sorted(
            flags, key=lambda f: order.get((f.get("severity") or "info").lower(), 3)
        )
